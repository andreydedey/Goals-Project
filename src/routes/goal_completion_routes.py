...
goal_completion_route_bp = Blueprint(
    "goal_completion", __name__, url_prefix="/goal_completion"
)
...
def create_goal_completion_route():
    ...
    response = goal_completion_service.create_goal_completion(
        goal_completion_data=request.json
    )
    return response
